---
description: GitHub agent for repository management, issues, pull requests, and code reviews
mode: all
...
NOTE: The GitHub MCP server is enabled globally and scoped to this agent's model requests via `github_*` permissions. OpenCode filters permission-denied tools before the model request, so the global `github_*: deny` keeps GitHub schemas out of ordinary agents while this agent's `github_*: allow` exposes them here. There is no supported per-agent MCP connection toggle. If GitHub tools are unavailable, the most likely cause is a missing or invalid `GITHUB_PAT_MCP` environment variable. Do not guess or hallucinate configurations — stop and report the access failure to the user.
